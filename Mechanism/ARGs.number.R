args.mges.number <- read.table("ARGs.number.txt",header = TRUE,sep = "\t",row.names = 1)
#载入绘图数据
...
#Time
png(filename = "ARGs.number.mechanism.time.tiff",width = 12000,height = 9000,res = 600,type = "cairo")
par(mar=c(6.3,5,4,10),family ="myFont1")
#设置绘图区
bar <- barplot(args.mges.number,names.arg = c(rep("",13)),col = brewer.pal(8,"Set3"),border = "black",family = "myFont1",xlim = c(0,15),axes = F,ylim = c(0,40))
#绘制第一张图
...
text(x = bar, y = -1.2,adj = c(1,0.8),labels = colnames(args.mges.number),cex = 2.4, font = 2,srt = 45)
#添加x轴
...
legend(0,1.08,legend = rownames(args.mges.number),fill = brewer.pal(9,"Set3"),bty = "n",ncol = 2,text.width = 0.3,cex = 2.5,text.font = 2)
#添加图例
...
text(x = 0.25,y = 0.85,labels = "Original soil",font = 2,cex = 2.5)
dev.off()

#Treatment
args.mges.number <- args.mges.number[,c(1,seq(from = 2, to = 11,by = 3),seq(from =3, to = 12, by = 3),seq(from = 4,to = 13, by = 3))]
#修改样品顺序
png(filename = "ARGs.number.mechanism.treatment.tiff",width = 12000,height = 9000,res = 600,type = "cairo")
par(mar=c(6.3,5,4,10),family ="myFont1")
#设置绘图区
bar <- barplot(args.mges.number,names.arg = c(rep("",13)),col = brewer.pal(8,"Set3"),border = "black",family = "myFont1",xlim = c(0,15),axes = F,ylim = c(0,40))
#绘制第一张图
...
par(family = "myFont1")
plot(0:1, 0:1, type="n", xlab="",ylab="", axes=FALSE)
legend(0,1.1,legend = rownames(args.mges.number),fill = brewer.pal(9,"Set3"),bty = "n",ncol = 2,text.width = 0.3,cex = 2.5,text.font = 2)
#添加图例
